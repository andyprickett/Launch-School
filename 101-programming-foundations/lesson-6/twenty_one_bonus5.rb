...
def calculate_result(player_total, dealer_total)
  if player_total > GAME_HIGH
    :player_busted
  elsif dealer_total > GAME_HIGH
    :dealer_busted
...
loop do
  ...
  loop do
    display_scores(scores)
    display_cards(player_hand, dealer_hand, whos_turn)
    break if dealer_total >= GAME_STAY
...
puts "Thanks for playing #{GAME_HIGH}. Goodbye."
