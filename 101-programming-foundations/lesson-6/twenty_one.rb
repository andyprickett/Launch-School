...
def stay?(hand)
  (17..21).cover?(calculate_total(hand))
end
...
def display_results(player_hand, dealer_hand, whos_turn)
  ...
  if result.to_s.include?('busted')
    ...
    display_winner(player_hand, dealer_hand, result)
  end
end
...
def display_winner(player, dealer, result)
  puts "Dealer has #{calculate_total(dealer)}."
  puts "You have #{calculate_total(player)}."
  case result
  when :player_busted
    puts "You busted. Dealer wins."
  when :dealer_busted
...
loop do
  ...
  whos_turn = 'player'
  stayed = while !busted?(player_hand)
             display_cards(player_hand, dealer_hand, whos_turn)
             player_turn_choice = nil
             loop do
               puts "Would you like to 'hit' or 'stay'?"
               player_turn_choice = gets.chomp
               break if ['hit', 'stay'].include?(player_turn_choice.downcase)
               puts "Try again..."
             end

             break(true) if player_turn_choice == 'stay'
  ...
  if stayed
    ...
    display_results(player_hand, dealer_hand, whos_turn)
